			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						ArrayList<게시판> list = bbsDAO.getList();
						for(int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%= list.get(i).get게시글번호() %></td>
						<td><a href="view.jsp?seq=<%= list.get(i).get게시글번호() %>"><%= list.get(i).get제목() %></a></td>
						<td><%= list.get(i).get고객아이디() %></td>
						<td><%= list.get(i).get작성일() %></td>
					</tr>
					<%
						}
					%>
